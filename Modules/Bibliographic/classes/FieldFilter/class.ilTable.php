...
class ilTable
{
    ...
    public function __construct(?object $a_parent_obj, ilBiblAdminFactoryFacade $facade)
    {
        $this->facade = $facade;
        ...
        $columns = [
            'position' => $f->table()->column()->text('order'),
            'identifier' => $f->table()->column()->text('identifier'),
            'translation' => $f->table()->column()->text('translation'),
            'standard' => $f->table()->column()->text('standard')
        ];
